...
"""
        ...
        if os.path.exists(script_path):
            print(f"Script {script_path} already exists, skipping generation")
...
"""
    ...
    def _parse_timing_report(self, corner: str) -> Optional[nx.DiGraph]:
        ...
        def process_hold_timing_path(file_path: str, delay_type: str):
            ...
            for match in path_sections:
                ...
                # print(f"start_reg: {start_reg}, end_reg: {end_reg}, arrival: {arrival_time}, hold: {hold_time}, path_detail: {path_detail}")
        ...
        def graph_edge_mark():   
            for u, v, data in graph.edges(data=True):
                ...
                setup_path = setup_info.get("path_detail", "N/A")
                hold_path = hold_info.get("path_detail", "N/A") 
                def extract_nodes(path_str: str):
                    """
                    从 path_detail 字符串中提取所有节点名（例如 _555_/Y）。
                    """
                    nodes = []
                    for line in path_str.splitlines():
                        match = re.search(r'[v\^]\s+(.+)/[A-Z]+', line)
                        if match:
                            nodes.append(match.group(1))
                    return nodes
                def has_shared_tail(setup_nodes, hold_nodes, min_shared=3):
                    """
                    判断hold_nodes是否是setup_nodes的子集
                    """
                    return all(node in setup_nodes for node in hold_nodes)
                
                setup_nodes = extract_nodes(setup_path)
                # print(f"setup_nodes: {setup_nodes}")
                hold_nodes = extract_nodes(hold_path)
                # print(f"hold_nodes: {hold_nodes}")

                if len(setup_nodes) <= 1 and len(hold_nodes) <= 1:
                    graph[u][v]['path_detail'] = 'a'
                elif setup_nodes == hold_nodes:
                    graph[u][v]['path_detail'] = 'c'
                elif has_shared_tail(setup_nodes, hold_nodes):
                    graph[u][v]['path_detail'] = 'd'
                else:
                    graph[u][v]['path_detail'] = 'b'
    ...
    def _run_sta_analysis(self, corner: str, liberty_path: List[str], path: List[Tuple[str, str]]) -> Optional[nx.DiGraph]:
        ...
        Returns:
            nx.DiGraph: 构建的有向图，如果失败则返回None
        """
        # 如果timing报告已存在，则直接返回解析后的图，无需重新运行STA
        max_rpt = f"{self.work_dir}/timing_{corner}_max.rpt"
        min_rpt = f"{self.work_dir}/timing_{corner}_min.rpt"
        if os.path.exists(max_rpt) and os.path.exists(min_rpt):
            print(f"{max_rpt} 和 {min_rpt} 已存在，跳过STA分析，直接解析报告。")
    ...
    def print_graph_info(self, corner: str) -> None:
        ...
        # 统计边类型分布并写入文件
        type_count = {'a': 0, 'b': 0, 'c': 0, 'd': 0, 'N/A': 0}
        edge_type_filename = os.path.join(self.work_dir if self.work_dir else '.', f"edge_type_stat_{corner}.txt")
        with open(edge_type_filename, 'w') as f:
            f.write('startpoint,endpoint,type\n')
            for u, v, data in graph.edges(data=True):
                t = data.get("path_detail", "N/A")
                if t in type_count:
                    type_count[t] += 1
                else:
                    type_count['N/A'] += 1
        ...
        for t, count in type_count.items():
            print(f"   - 类型 {t}: {count} 条")
        print(f"边类型明细已写入: {edge_type_filename}")
        ...
        for u, v, data in graph.edges(data=True):
            ...
            path_detail = data.get("path_detail", "N/A")
            ...
            print(f"   {u:<40} {v:<40} {setup_arrival:<15} {hold_arrival:<15} {setup_library:<15} {hold_library:<15} {path_detail:<15}")
    ...
    def modify_timing_constraint_graph(self, corner: str) -> Optional[nx.DiGraph]:
        """
        根据边的类型修改timing constraint graph，添加相应的节点
        
        Args:
            corner: 工艺角名称
            
        Returns:
            nx.DiGraph: 修改后的timing constraint graph
        """
        if corner not in self.graphs:
            print(f"错误：未找到工艺角 {corner} 的图")
            return None
            
        original_graph = self.graphs[corner]
        # 创建新的图，用于存储修改后的结果
        ...
        for u, v, data in original_graph.edges(data=True):
            edge_type = data.get('path_detail', 'a')
            setup_info = data.get('setup_delay', {})
            hold_info = data.get('hold_delay', {})
            
            if edge_type == 'a':
                # 类型a: 无延迟可插入，保持原边
                modified_graph.add_edge(u, v, **data)
                
            elif edge_type == 'b':
                # 类型b: ps、ph独立，添加两个独立节点us和uh
                us_node = f"{u}_{v}_us"
                uh_node = f"{u}_{v}_uh"
                
                # 添加新节点
                modified_graph.add_node(us_node, type='inserted_setup', original_edge=(u, v))
                modified_graph.add_node(uh_node, type='inserted_hold', original_edge=(u, v))
                
                # 重新连接边
                # u -> us (延迟为0)
                modified_graph.add_edge(u, us_node, 
                                      setup_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      path_detail='setup_path',
                                      original_edge=(u, v))
                # us -> v (延迟为原本的setup constraint)
                modified_graph.add_edge(us_node, v,
                                      setup_delay=setup_info,
                                      path_detail='setup_constraint',
                                      original_edge=(u, v))
                
                # u -> uh (延迟为0)
                modified_graph.add_edge(u, uh_node,
                                      hold_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      path_detail='hold_path',
                                      original_edge=(u, v))
                # uh -> v (延迟为原本的hold constraint)
                modified_graph.add_edge(uh_node, v,
                                      hold_delay=hold_info,
                                      path_detail='hold_constraint',
                                      original_edge=(u, v))
                
            elif edge_type == 'c':
                # 类型c: ps = ph，添加一个共同节点us
                us_node = f"{u}_{v}_us"
                
                # 添加新节点
                modified_graph.add_node(us_node, type='inserted_common', original_edge=(u, v))
                ...
                modified_graph.add_edge(u, us_node,
                                      setup_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      hold_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      path_detail='common_path',
                                      original_edge=(u, v))
                # us -> v (延迟为原本的setup/hold constraint，因为ps = ph)
                modified_graph.add_edge(us_node, v,
                                      setup_delay=setup_info,
                                      hold_delay=hold_info,
                                      path_detail='common_constraint',
                                      original_edge=(u, v))
                
            elif edge_type == 'd':
                # 类型d: ps >= ph，添加两个节点us和uh，但有依赖关系
                us_node = f"{u}_{v}_us"
                uh_node = f"{u}_{v}_uh"
                
                # 添加新节点
                modified_graph.add_node(us_node, type='inserted_setup_dominant', original_edge=(u, v))
                modified_graph.add_node(uh_node, type='inserted_hold_dependent', original_edge=(u, v))
                
                # 重新连接边，hold路径可能经过setup节点
                # u -> us (延迟为0)
                modified_graph.add_edge(u, us_node,
                                      setup_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      path_detail='setup_path',
                ...
                # us -> uh (延迟为0, 确保ps >= ph的约束)
                modified_graph.add_edge(us_node, uh_node,
                                      setup_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      hold_delay={"arrival_time": 0.0, "library_time": 0.0, "path_detail": "inserted_zero_delay"},
                                      path_detail='setup_to_hold_constraint',
                                      original_edge=(u, v))
    ...
    def print_modified_graph_info(self, corner: str, modified_graph: nx.DiGraph) -> None:
        """
        打印修改后的图信息
        
        Args:
            corner: 工艺角名称
            modified_graph: 修改后的图
        """
        print(f"\n{'='*50}")
        print(f"工艺角 {corner} 的修改后图信息:")
        print(f"{'='*50}")
        
        # 统计不同类型的节点
        node_types = {}
        for node, data in modified_graph.nodes(data=True):
            node_type = data.get('type', 'original')
        ...
        for u, v, data in modified_graph.edges(data=True):
            edge_type = data.get('path_detail', 'unknown')
        ...
        print(f"\n3. 基本信息:")
        print(f"   - 总节点数: {len(modified_graph.nodes())}")
        print(f"   - 总边数: {len(modified_graph.edges())}")
        
        # 保存修改后的图信息到文件
        modified_graph_filename = os.path.join(self.work_dir if self.work_dir else '.', f"modified_graph_{corner}.txt")
        with open(modified_graph_filename, 'w') as f:
            f.write('节点信息:\n')
            f.write('node_name,type,original_edge\n')
            for node, data in modified_graph.nodes(data=True):
                original_edge = data.get('original_edge', 'N/A')
                if original_edge != 'N/A':
                    original_edge = f"{original_edge[0]}->{original_edge[1]}"
                f.write(f'{node},{data.get("type", "original")},{original_edge}\n')
            
            f.write('\n边信息:\n')
            f.write('source,target,path_detail,original_edge\n')
            for u, v, data in modified_graph.edges(data=True):
                original_edge = data.get('original_edge', 'N/A')
                if original_edge != 'N/A':
                    original_edge = f"{original_edge[0]}->{original_edge[1]}"
                f.write(f'{u},{v},{data.get("path_detail", "unknown")},{original_edge}\n')
        
        print(f"\n修改后的图详细信息已保存到: {modified_graph_filename}")
    ...
    try:
        ...
        for corner in corners_config.keys():
            ...
            if modified_graph is not None:
                builder.print_modified_graph_info(corner, modified_graph)
            else:
                print(f"修改 {corner} 工艺角的图失败")
