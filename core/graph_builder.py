...
from core.run_ssta import _run_ssta
...
"""
    ...
    def _create_ssta_script(self, corner: str, mode: str, liberty_path: List[str], path: List[Tuple[str, str]]) -> str:
        """
...
"""
        # 为每个路径生成max delay报告
            for start, end in path:
                script += f"report_checks -from {start} -to {end} -path_delay max -format full_clock_expanded \n"
...
"""
        # 为每个路径生成min delay报告
            for start, end in path:
                script += f"report_checks -from {start} -to {end} -path_delay min -format full_clock_expanded \n"
        #>> {self.work_dir}/timing_{corner}_min.rpt
        return script
    ...
    def _run_sta_analysis(self, corner: str, liberty_path: List[str], path: List[Tuple[str, str]]) -> Optional[nx.DiGraph]:
        ...
        with open(script_path, 'w') as f:
            f.write(self._create_sta_script(corner, 'setup', liberty_path, path))
    ...
    def _run_ssta_analysis(self, corner: str, liberty_path: List[str], path: List[Tuple[str, str]]) -> Optional[nx.DiGraph]:
        """
        运行OpenSTA时序分析并构建图
        ...
        """
        # 创建临时TCL脚本
        script_setup_path = f"{self.work_dir}/temp_{corner}_setup.tcl"
        with open(script_setup_path, 'w') as f:
            f.write(self._create_ssta_script(corner, 'setup', liberty_path, path))
        script_hold_path = f"{self.work_dir}/temp_{corner}_hold.tcl"
        with open(script_hold_path, 'w') as f:
            f.write(self._create_ssta_script(corner, 'hold', liberty_path, path))
        script_path = script_setup_path.replace("_setup.tcl", ".tcl")

        print("运行OpenSTA for ssta")
        setup_info = self._run_sta_lib(script_setup_path)
        hold_info = self._run_sta_lib(script_hold_path)
        lib_info = {}
        for key,value in setup_info.items():
            lib_info[key] = (setup_info[key],hold_info[key])
        #print(f"lib_info: {lib_info}")
        info_path = script_path.replace(".tcl", "_tp.txt")
        if not os.path.exists(info_path):
            _run_ssta(script_path, 5000, 'mle', lib_info)
        
        graph = self.build_timing_graphs(info_path)
        if graph is not None:
            self.graphs[corner] = graph
            
        
    
    def _run_sta_lib(self,input_file):
        try:
            # 运行OpenSTA
            result = subprocess.run(['/home/wllpro/llwang07/kxzhu/ssta/OpenSTA_D/app/sta', '-exit', input_file], 
                                 capture_output=True, 
                                 text=True,
                                 cwd=self.work_dir)
            
        except Exception as e:
            print(f"Error running STA for: {str(e)}")
        return self.parse_holdsetup(result.stdout)

    def parse_holdsetup(self,output):
        timing_paths = {}
        current_path = {}
        lines = re.split(r'\n\n', output)
        for line in lines:
            ...
            match_lib =  re.search(r"\s*(-?\d+\.\d+)\s*(-?\d+\.\d+)\s*(library hold time|library setup time)", line)
            ...
            if match_lib:
                delay_lib = float(match_lib.group(1))
            if match_slack:
                key = (current_path['startpoint'], current_path['endpoint'])
                timing_paths[key] = delay_lib
                current_path = {}
                delay_lib = 0.0

        return timing_paths
        
    def run_timing_graphs_info(self):
        """
        ...
        for corner, liberty_path in self.corners_config.items():
            self._run_ssta_analysis(corner, liberty_path, path)
            
        
    
    def build_timing_graphs(self, path: str) -> Dict[str, nx.DiGraph]:
        """
        构建所有工艺角的timing graphs
        ...
        for line in lines:  
            ...
            start_reg = parts[0]
            end_reg = parts[1]
            max_delay = (float(parts[2][1:-1]),float(parts[3][:-1]),float(parts[4][:-1]))
            min_delay = (float(parts[5][1:-1]),float(parts[6][:-1]),float(parts[7][:-1]))
            setup_time = float(parts[8])
            hold_time = float(parts[9])
            ...
            if graph.has_edge(start_reg, end_reg):
                graph[start_reg][end_reg]['setup_delay'] = {
                    "arrival_time": max_delay,
                    "library_time": setup_time,
                }
                graph[start_reg][end_reg]['hold_delay'] = {
                    "arrival_time": min_delay,
                    "library_time": hold_time,
                }
            else:
                graph.add_edge(start_reg, end_reg, **{
    ...
    def print_graph_info(self, corner: str) -> None:
        ...
        # 边信息(时序路径)
        print(f"\n2. 时序路径信息:")
        print(f"   {'起点寄存器':<40} {'终点寄存器':<40} {'Setup Delay':<40} {'Hold Delay':<40} {'Setup Library':<15} {'Hold Library':<15}")
        print(f"   {'-'*190}")
        ...
        for u, v, data in graph.edges(data=True):
            ...
            # 处理元组格式化
            if isinstance(setup_arrival, tuple):
                setup_arrival_str = f"({setup_arrival[0]:.3f}, {setup_arrival[1]:.3f}, {setup_arrival[2]:.3f})"
            else:
                setup_arrival_str = "N/A"
                
            if isinstance(hold_arrival, tuple):
                hold_arrival_str = f"({hold_arrival[0]:.3f}, {hold_arrival[1]:.3f}, {hold_arrival[2]:.3f})"
            else:
                hold_arrival_str = "N/A"
                
            if isinstance(setup_library, (int, float)):
                setup_library_str = f"{setup_library:.3f}"
            else:
                setup_library_str = "N/A"
                
            if isinstance(hold_library, (int, float)):
                hold_library_str = f"{hold_library:.3f}"
            else:
                hold_library_str = "N/A"
                
            print(f"   {u:<40} {v:<40} {setup_arrival_str:<40} {hold_arrival_str:<40} {setup_library_str:<15} {hold_library_str:<15}")
