...
def find_min_TCLK(corner_graphs, TCLK_min=0, TCLK_max=1000, tol=0.1, max_attempts=50):
    """
    二分查找最小可行的时钟周期
    ...
    # 首先验证TCLK_max是否可行
    p, setup_padding, hold_padding, converged, msg = run_dual_delay_padding(corner_graphs, TCLK_max)
    if not converged:
    ...
    while TCLK_max - TCLK_min > tol and attempts < max_attempts:
        ...
        p, setup_padding, hold_padding, converged, msg = run_dual_delay_padding(corner_graphs, mid_TCLK)
