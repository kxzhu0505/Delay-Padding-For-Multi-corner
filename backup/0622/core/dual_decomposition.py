...
def run_dual_delay_padding(corner_graphs, TCLK, max_iter=100, tol=1e-3, alpha=0.1, beta=0.1, max_padding=float('inf')):
    """
    ...
    def check_setup_timing(G, p, setup_padding, edge_key=None):
        ...
        for u, v in edges:
            ...
            if setup_data:
                arrival = setup_data.get("arrival_time", 0)
                T_setup = setup_data.get("library_time", 0)
                setup_slack = (TCLK + T_setup - arrival) - (p[v] - p[u]) - setup_padding[(u,v)]
                if setup_slack < -tol:
